...
theta0_guess  = np.array([0.])
sigma0        = np.array([0.05])

for i in range(alpha_grp):
    #print("alpha_grp", i)
    aidx       = int(i * size + rank)
    geo_coeffs = vmec_fieldlines(vs, rho_arr[mpi.group], alpha_scan_guess[aidx], theta1d=theta)
    ...
    for j in range(ntheta0_guess):
        ...
        temp_RE, X_arr,_,_,_,_   = gamma_ball_full(dPdrho, theta, bmag, gradpar, cvdrift_fth, gds2_fth, vguess, temp_RE_guess) 
        vguess                   = X_arr[1:-1]
...
if rank == 0:
    ...
    if mpi.group == 0: # Collecting all the growth rates from different surfaces/mpi.groups and 
        ...
        Ltc0       = np.load(path1 + "/ball_gam{0}.npy".format(int(dof_idx)), allow_pickle=True)
        theta0tc0  = np.load(path1 + "/ball_theta0{0}.npy".format(int(dof_idx)), allow_pickle=True)
        alphatc0   = np.load(path1 + "/ball_alpha{0}.npy".format(int(dof_idx)), allow_pickle=True)
