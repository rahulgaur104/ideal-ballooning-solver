...
def check_ball(shat_n, alpha_n, theta0):
    ...
    g = (
        1
        + (
            shat_n * (theta_ball - theta0)
            - alpha_n * (np.sin(theta_ball) - np.sin(theta0))
        )
        ** 2
    )
    c = alpha_n * (
...
shat_grid = np.linspace(0.0, 2, len1)
alpha_grid = np.linspace(0.0, 1.2, len2)
...
results1 = np.array(
    [
        [
            pool.apply_async(
                check_ball, args=(shat_grid[i], alpha_grid[j], theta0_arr[0])
            )
            for i in range(len1)
        ]
        for j in range(len2)
...
results2 = np.array(
    [
        [
            pool.apply_async(
                check_ball_long, args=(shat_grid[i], alpha_grid[j], theta0_arr[1])
            )
            for i in range(len1)
        ]
        for j in range(len2)
...
results3 = np.array(
    [
        [
            pool.apply_async(
                check_ball_long, args=(shat_grid[i], alpha_grid[j], theta0_arr[2])
            )
            for i in range(len1)
        ]
        for j in range(len2)
...
cs = plt.contour(
    Y,
    X,
    np.logical_or(np.logical_or(ball_scan_arr1, ball_scan_arr2) + 0, ball_scan_arr3),
    linewidths=2.5,
    colors="black",
    levels=[0.0],
)
