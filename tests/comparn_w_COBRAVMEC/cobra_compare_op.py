...
A = np.loadtxt("cobra_grate.NCSX_op")
...
gamma2 = np.load("gamma_max_op.npy")
s2 = np.linspace(0.01, 0.995, len(gamma2))
...
gamma_scaling = np.min(gamma1)/np.min(gamma2)
...
plt.tight_layout()
plt.savefig("NCSX_stable_comparison.png", dpi=600)
plt.show()
